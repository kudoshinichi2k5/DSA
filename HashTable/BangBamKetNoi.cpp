...
{
    int h_key = Hash(ht, x.Maso);
    for(Node* curNode = ht.table[h_key].head; curNode != NULL; curNode = curNode->next)
    {
        if(curNode->data.Maso == maso)      return curNode;
    }
    return NULL;
}
...
int Delete(Hashtable &ht, int maso)
{
    int h_key = Hash(ht, x.Maso);
    Node* prev = NULL;
    Node* curNode = ht.table[h_key].head;
    while(curNode!=NULL)
    {
        if(curNode->data.Maso == maso)
        {
